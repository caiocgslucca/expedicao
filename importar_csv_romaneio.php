...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            if ($column[1] == 'Nro. Documento' ||  $column[0] == '' || $column[0] == 'Nro. Entrega' )
                continue;
            ...
            $sqlInsert = "INSERT INTO expedicao VALUES( null, '$column[1]', '$column[3]', '$column[5]', '$column[9]', 
             '$column[18]', '$column[37]', '$column[38]', '$column[50]', '$column[51]', '$column[84]', null, '$usuario', '$datahora' , null )";
...
<?php
$qsl_valida = "SELECT MAX(id) as id_valida FROM `expedicao` where finalizado IS NULL;";
...
while ($rows = mysqli_fetch_assoc($recebidos)) {
    ...
    if( $id == "" ){
        ...
        ?>
        <li class="nav-item">
                    <a class="nav-link active" href="limpar_base_romaneio">Limpar base que foi importada errada.</a>
                </li>
        <?php
    }
    ?> 
    ...
    <form class="form-horizontal" action="importar_csv_romaneio" method="post" name="frmCSVImport" id="frmCSVImport" enctype="multipart/form-data">
